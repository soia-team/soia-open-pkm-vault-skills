...
该脚本只负责底线：source 概念覆盖、报告长度、deck 页数、信息块数、题目/答案数量、卡片/表格数量、音视频脚本长度。通过脚本不代表视觉已经专业；仍要人眼检查排版、层级、可读性和是否像目标媒介。
...
| 参考图遵循 | 只复用颜色、没有复现构图语法 | 必须保留参考图的核心结构，如路径、视觉链、结论区 |
| 术语地图 / 路线图 | 连续同尺寸文字卡片、没有视觉主线 | 直接判定不合格；必须有路径或视觉隐喻链 |
| 编辑型地图文字 | 字体越来越小以容纳全文 | 节点正文 ≤ 35 字、底部结论 ≤ 55 字，超出则拆分或移出图片 |
