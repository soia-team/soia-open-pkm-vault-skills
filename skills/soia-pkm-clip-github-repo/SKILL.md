...
```bash
npx skills add soia-team/soia-open-pkm-clip-skills -g -a '*' -s soia-pkm-clip-github-repo -y
```

配置约定：

```text
~/.config/soia-skills/soia-pkm-clip-github-repo/config.yml
SOIA_PKM_CLIP_GITHUB_REPO_CONFIG_FILE=<custom-config-path>
# 兼容别名：$SOIA_PKM_CLIP_REPO_CONFIG_FILE（以及原有 $SOIA_PKM_CLIP_REPO_ENV_FILE）
```
...
- 私有 `config.yml`（可选，**不要放在 vault 或开源 skill 仓库**）：
  ...
  优先级：`$SOIA_PKM_CLIP_GITHUB_REPO_CONFIG_FILE`（兼容别名 `$SOIA_PKM_CLIP_REPO_CONFIG_FILE`，以及原有 `$SOIA_PKM_CLIP_REPO_ENV_FILE`）> `~/.config/soia-skills/soia-pkm-clip-github-repo/config.yml`。
...
**交付顺序**：先把项目卡/调研笔记落盘，再输出下面的回执，不得反过来；自动字段抓取失败（如非 git 目录拿不到最近提交）在回执里显式标注"未核实"，不编造。
