...
- 把 `TELEGRAM_API_ID` / `TELEGRAM_API_HASH` / `TELEGRAM_SESSION_STRING` 放进私有 `config.yml`（`$SOIA_PKM_CLIP_X_CONFIG_FILE` 或 `~/.config/soia-skills/soia-pkm-clip-x/config.yml`），不要放进 vault 或提交到开源 skill 仓库
