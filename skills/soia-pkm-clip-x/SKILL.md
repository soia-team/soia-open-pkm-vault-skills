...
```bash
npx skills add soia-team/soia-open-pkm-vault-skills -g -a '*' -s soia-pkm-clip-x -y
```

配置约定：

```text
~/.config/soia-skills/soia-pkm-clip-x/config.yml
...
- 文章目录定位优先级：
  ...
  3. 退回通用默认 `Articles`；脚本会向 stderr 打印以下警告，退出码保持 0：`WARN: 未找到 articles 目录配置（--articles-dir / OBSIDIAN_ARTICLES / config.yml），已落默认 Articles/——该目录不是本 vault 的正式归档位，请确认或配置`
...
- 私有 `config.yml`（可选，**不要放在 vault 或开源 skill 仓库**）：
  ...
  优先级：`$SOIA_PKM_CLIP_X_CONFIG_FILE`（或兼容别名 `$SOIA_PKM_CLIP_X_ENV_FILE`）> `~/.config/soia-skills/soia-pkm-clip-x/config.yml`。
