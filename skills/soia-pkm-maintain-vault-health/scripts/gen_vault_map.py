#!/usr/bin/env python3
"""
gen_vault_map.py — 生成 Markdown vault 的叶级地图（soia-pkm-maintain-vault-health）
...
重新生成：使用 soia-pkm-maintain-vault-health。
"""
import argparse
import datetime
import os
...
def build_map(root):
    ...
    def walk(path, rel, depth):
        ...
        dirs = [
            ...
            and not os.path.islink(os.path.join(path, e))
...
def main():
    ...
    configured_output = args.output or os.environ.get("SOIA_VAULT_MAP_OUTPUT")
    output = configured_output or os.path.join(vault, DEFAULT_RELATIVE_OUTPUT)
