...
- 每个写入型技能新建或修改 `.base` 时，必须至少用一个 `file.inFolder("<vault-relative-directory>")` 声明可验证的目录范围；目录必须真实存在。
- `file.hasTag(...)`、字段条件和视图过滤只能在目录范围内继续细分，不能代替 `file.inFolder`。需要同时使用目录与标签时，把它们放进同一个 `filters.and`。
- 既有 tag-only Base 一旦进入本次写入或验证范围，先补齐目录边界，再运行 `vault_index_verify.py`；不得通过放宽路径验证器掩盖问题。
