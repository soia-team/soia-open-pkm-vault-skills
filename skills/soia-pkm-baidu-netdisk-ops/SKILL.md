---
name: soia-pkm-baidu-netdisk-ops
description: 百度网盘原子操作与只读 JSONL 扫描适配。触发：「看下百度网盘」「登录百度网盘」「扫描百度网盘」
dependencies:
  external:
    - name: baidu-drive
      required: true
...
```bash
npx skills add soia-team/soia-open-pkm-clip-skills -g -a '*' -s soia-pkm-baidu-netdisk-ops -y
```
...
- 静态：`quick_validate.py skills/soia-pkm-baidu-netdisk-ops`、`scripts/audit_skills.py`、`git diff --check`。
