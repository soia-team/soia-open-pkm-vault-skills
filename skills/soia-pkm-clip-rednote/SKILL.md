...
```text
~/.config/soia-skills/soia-pkm-clip-rednote/config.yml
...
归档落盘只代表 `captured`；单对象默认继续走生命周期管理的 `organized → MOC/导航 → map → Base` 五段合同，逐项回执，不适用阶段写 `not_applicable`。
...
- 私有 `config.yml`（可选，**不要放在 vault 或开源 skill 仓库**）：
  ...
  优先级：`$SOIA_PKM_CLIP_REDNOTE_CONFIG_FILE`（或兼容别名 `$SOIA_PKM_CLIP_REDNOTE_ENV_FILE`）> `~/.config/soia-skills/soia-pkm-clip-rednote/config.yml`。
