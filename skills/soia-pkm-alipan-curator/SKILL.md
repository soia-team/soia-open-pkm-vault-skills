...
其他可识别说法包括「整理云盘」「云盘盘点」「更新云盘索引」「更新 Excel 总索引」「生成家庭导航 Excel」「给云盘建图书馆」；单个文件的浏览、下载或上传交给 `soia-pkm-alipan-drive-ops`。
...
```bash
npx skills add soia-team/soia-open-pkm-vault-skills -g -a '*' -s soia-pkm-alipan-curator -y
```

配置约定：

```text
~/.config/soia-skills/soia-pkm-alipan-curator/config.yml
...
- 阿里云盘登录凭据留在 provider 自己的登录态或系统凭据存储中；本技能只通过原子层调用，不读取、复制或打印秘密值。
- 移动计划、运行包、发布 manifest 和审计报告属于可追溯状态，放在用户 state 目录；不得写入仓库、vault `outputs/` 或某个 AI 的会话目录。
- 可重建的扫描解析与 Excel 增量缓存放用户 cache 目录；下载样本和渲染探针放 OS 临时目录并在成功或失败后清理。
- Excel、家庭导航和学习计划属于用户交付物，只写用户指定目录、私有配置目录或本技能约定的 Downloads 回退目录。
- 日志和回执只记录脱敏后的路径角色、数量、哈希和状态；不暴露用户名、本地私有绝对路径、账号标识或云盘凭据。
...
**大型整理固定执行链**：统一按 `merge map → assign numbers (optional) → build reclass → build structure → preflight → audit → dry-run → execute → fresh terminal scan → refresh OB/Excel → publish cloud catalog → final audit` 顺序推进；任何一步失败都停在该步，不得跳过门禁直接写云盘。多 agent 只可并行做只读盘点、内容审核、计划生成和独立验证；`mkdir` / `mv` / `rename` / 回收站删除 / 上传等云盘写入始终由一个写入者执行，批量入口显式使用 `--max-parallel 1`。计划和结构合同合并后，写入者还必须按 file_id 做迁移后终态扫描，确认空壳确实存在且子项为 0，再按已批准清单送回收站。只要本轮存在结构写入，运行包在云端索引发布证据通过前只能标记 `pending_catalog_publish`，不得提前写 `completed`。
...
| 已批准重分类方案的可恢复执行 | `scripts/apply_reclass.py` | 读取带 `action_id` 的 JSONL 计划；`mv`/`rename` 必须携带 `file_id`，写前、终态与 resume 都按 `name → file_id` 回读核验；写调用默认节流 2 秒（`--throttle-ms 0` 关闭），限流按 30/60/120 秒退避；限定 `--root` 与可选 `--archive-root`、默认 dry-run、失败即停并写 verified 账本 |
| 大批同源同目标移动的可恢复执行 | `scripts/apply_reclass_bulk.py` | 沿用相同计划/边界/身份/账本合同；最多 20 项合并一次 `mv`，批前批后各读源与目标并逐 action 以 `file_id` 记终态；写调用默认节流 2 秒（`--throttle-ms 0` 关闭），限流按 30/60/120 秒退避；云盘写入始终单写者，`--max-parallel` 只能是 `1`；默认 dry-run |
...
| 索引发布闭环审计 | `scripts/audit_catalog_publish.py` | 从运行包内发布 manifest 检查统一版本/时间、分区覆盖、本地与远端 SHA1/字节/file_id、唯一同名文件、旧 ID 零引用和幂等状态；失败时返回非零退出码 |
...
- 每次实际发布只生成一次 release metadata，并让 `00_馆藏总览.md`、全部 `_全文检索/*.md`、总 Excel、全部分区 Excel 和现行资源地图共享同一组 `catalog_release_id`、`index_updated_at`、`snapshot_at`、`catalog_schema_version` 与 `source_fingerprint`。时间必须带时区；相同输入复跑必须 `unchanged`，不能用“当前时间”制造新版本。
...
- Excel 作者层不绑定单一平台：优先复用宿主提供的 `@oai/artifact-tool`（格式更丰富，自带公式错误扫描与截图预览）；缺失时自动退回内置的 `openpyxl` 通用兜底，保证没有该运行时访问权限的 AI/宿主也能产出数据口径一致的有效索引。`--renderer {auto,artifact-tool,openpyxl}` 显式选择或强制某条路径；`auto`（默认）优先用可用的 artifact-tool。交付验收时加 `--verify`，有 `soffice` 时传入以预计算链接显示值；openpyxl 路径下 `--soffice` 还用于补齐公式错误扫描，缺失时如实标注该轮 QA 未覆盖，不冒充已扫描通过。
- 分区需要面向家长的简明说明、筛选表和课程直达链接时，家庭导航输入必须来自终态扫描；传 `--soffice` 预计算 `HYPERLINK` 显示值，避免在线预览只显示公式。
- 家庭导航先上传并复核，再做终态扫描与总索引；否则索引会少算刚上传的文件。
- 家庭导航生成、上传并验证后，**必须显式向用户询问**是否要用 `scripts/check_family_nav_staleness.py register` 登记过期检测基准（不是可以默认跳过的隐藏步骤），并把注册表的绝对路径明确告知用户；同意后续以新鲜文件级扫描运行 `check` 检测疑似过期。该脚本只报告差异，不能自动重生成或上传，详见 `references/family-navigation-excel.md`。
...
- 结构写入后的云端索引上传是强制收官步骤，不是可选建议。本地生成后必须把总入口和所有受影响分区明细重新上传，独立 `ll` 记录最终 file_id/SHA1/bytes 和同名数量，再用 `audit_catalog_publish.py --final` 复核；缺少发布 manifest、远端证据、版本一致性或旧 file_id 零引用证明时，`audit_run_bundle.py --final` 必须失败。
...
4. **结构闭环 + 地图/总览刷新 + 云端发布 + 文档回填**：终态扫描后先跑编号/导览/关键云端产物/长系列分组/待确认项审计，违规数为 0；再重建受影响地图（尤其跨盘导致 file_id 全换时），把最终 file_id 写入 `resource_maps` 合同并验证真实 Markdown 直达链接。随后用同一 release metadata 生成 OB、总 Excel 和分区 Excel，重新上传云端索引并独立核对 file_id/SHA1/bytes/唯一同名文件。收官必须依次更新 OB 资源地图、Excel、云盘索引和 `01_先看这里`，并回填方案文档变更史；任何一个消费端未更新都不算完成

特大模块在上述四步外增加运行包门禁：`run.json` 中的所有 `focus_targets` 必须同时出现在初始扫描、内容审计和终态扫描；所有批次 action 均有 verified/带原因 skipped 结果；`structure-audit.json`、`catalog-publish-audit.json` 与 `ai-review.json` 均通过；最后执行 `scripts/audit_run_bundle.py --run-dir <run-dir> --final`。有结构写入但未发布索引时状态必须保持 `pending_catalog_publish`；只有云端发布审计通过后才允许改为 `completed`。运行时证据留在 XDG state，OB 只保存资源地图、冻结审计与短回执。
