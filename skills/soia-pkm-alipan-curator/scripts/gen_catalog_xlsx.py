...
def default_config_file() -> Path:
    ...
    return base / "soia-skills" / "soia-pkm-alipan-curator" / "config.yml"
...
def configured_output_dir() -> str | None:
    if os.environ.get(CONFIG_FILE_ENV_NAME):
        config_path = Path(os.environ[CONFIG_FILE_ENV_NAME]).expanduser()
    else:
