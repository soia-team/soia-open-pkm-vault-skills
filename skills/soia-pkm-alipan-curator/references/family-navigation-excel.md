...
6. **向用户显式询问是否要开启过期检测**：“这份导航已生成并核验通过，要不要登记一个检测基准，方便以后判断这个目录内容变了要不要重新生成？”——这不是可以默认跳过的可选步骤，是每次生成流程的固定检查点；用户同意后按下方“过期检测”一节执行 `register`，基准文件路径要明确告诉用户（不要静默写进用户不知道的目录）。用户拒绝就跳过，不强行注册。
7. 再做分区终态扫描、OB 资源地图、全文检索与总索引 Excel。

如果在第 7 步之后才上传导航表，刚生成的索引会少 1 个文件。
...
`gen_family_nav_xlsx.mjs` 本身不会自动更新——云盘内容变了，导航文档不会自动跟着变，只能靠人工/AI 重新走一遍流程。`check_family_nav_staleness.py` 提供一个轻量指纹检测工具，缓解“不知道什么时候该刷新”的问题。**每次生成流程走到“生成前后的正确顺序”第 6 步时，都要向用户显式询问是否要注册这次的基准**——不要因为这一节标着技术细节就自己悄悄决定开不开，也不要把它当成脚本作者才知道的隐藏能力。

同意后，用当次用于生成该 guide 的新鲜文件级 scan 登记基准指纹，并把注册表的**绝对路径明确回复给用户**（比如“基准已记到 `~/.local/state/soia-pkm-alipan-curator/family-nav-registry.json`”），不要只在日志里一带而过。注册表应放在用户自有的 state 目录或其他持久化位置，不要放进本仓库：

```bash
python3 '<skill-dir>/scripts/check_family_nav_staleness.py' register \
  --registry '<state-dir>/family-nav-registry.json' \
  --guide-id '<guide-id>' \
  --scope-root '/<云盘分区路径>' \
  --scan '<run-dir>/fresh-scan.jsonl' \
  --nav-file-path '/<云盘分区路径>/01_先看这里' \
  --generated-at '<ISO8601时间戳>'
```

后续取得覆盖相同范围的新鲜文件级 scan 后，可检查全部已登记 guide，或重复传入 `--guide-id` 只检查指定项：

```bash
python3 '<skill-dir>/scripts/check_family_nav_staleness.py' check \
  --registry '<state-dir>/family-nav-registry.json' \
  --scan '<run-dir>/fresh-scan.jsonl' \
  [--guide-id '<guide-id>'] \
  [--json]
...
`fresh` 表示轻量指纹未变化，`stale` 表示文件数、目录数、总字节数或双方均存在的最大修改时间至少一项不同，`unknown` 表示本次 scan 未覆盖该 guide 的 `scope_root`。检测到 `stale` 只说明导航疑似过期；人工或 AI 必须重新执行“真实内容审计 → 生成 → 上传 → 验证”全流程，不能根据检测结果自动选择 `resource_roots`、改写导航或上传文件。

这不是自动化后台服务，而是供下次会话、人工操作或定时任务主动调用的检测工具。它只检测和报告，绝不触发重新生成或上传。
