...
```bash
npx skills add soia-team/soia-open-pkm-clip-skills -g -a '*' -s soia-pkm-clip-douyin -y
```
...
- 私有 `config.yml`（可选，**不要放在 vault 或开源 skill 仓库**）：
  ...
  优先级：`$SOIA_PKM_CLIP_DOUYIN_CONFIG_FILE`（或兼容别名 `$SOIA_PKM_CLIP_DOUYIN_ENV_FILE`）> `~/.config/soia-skills/soia-pkm-clip-douyin/config.yml`。
