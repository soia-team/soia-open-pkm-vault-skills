...
OVERRIDE_CONFIG_NAME = "SOIA_PKM_CLIP_DOUYIN_CONFIG_FILE"
OVERRIDE_ENV_NAME = "SOIA_PKM_CLIP_DOUYIN_ENV_FILE"
...
def default_config_file() -> Path:
    ...
    return base / "soia-skills" / "soia-pkm-clip-douyin" / "config.yml"
...
def write_run_report(payload: dict[str, object], prefix: str = "archive_douyin") -> Path:
    """Persist a disposable run report to the platform temp dir and return its path.

    Run reports are use-once-then-discard diagnostics, not an audit trail, so
    they belong under the platform temp directory (Category A in
    DATA_STORAGE_SPEC.md) instead of the caller's cwd, which may be the vault
    root, a read-only dir, or anything else at call time.
    """
    base_dir = Path(tempfile.gettempdir()) / "soia-pkm-clip-douyin"
    base_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    report = base_dir / f"{prefix}-{timestamp}.json"
    report.write_text(
