...
# 这个 wrapper 负责「先调用用户原有的 notify 命令，再触发最小化 vault 会话日志脚本」。
#
# 接入方式（写进 config.toml，见 ../references/session-log-setup.md）：
#   notify = ["<installed-skill-path>/scripts/codex_notify_wrapper.sh", "--vault", "<vault-path>", "--log-dir", "<vault-relative-log-dir>"]
#
...
# 触发 vault 会话日志脚本；hook 失败不应影响 Codex 主流程。
